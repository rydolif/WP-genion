		...
		<section class="comparison">
			<div class="comparison__container container">
				<div class="comparison__grid">
					...
					<div class="comparison__filter">
						<h4 class="comparison__elem--title">Модель</h4>
						<p class="comparison__elem">Поверхность</p>
						<p class="comparison__elem">Автоматическое распыление жидкости</p>
						<p class="comparison__elem">Увеличение скорости уборки в 2 раза</p>
						<p class="comparison__elem">Автоматическое оприделение границ</p>
						<p class="comparison__elem">Автоматическое оприделение траектории</p>
						<p class="comparison__elem">Резервное питание</p>
						<p class="comparison__elem">Обём резервуара для житкости</p>
						<p class="comparison__elem">Мощность всасывания</p>
						<p class="comparison__elem">Потребляемая мощность</p>
						<p class="comparison__elem">Время роботы</p>
						<p class="comparison__elem">Уровень шума</p>
						<p class="comparison__elem">Емкость батареи</p>
						<p class="comparison__elem">Количество сменых салфеток</p>
						<p class="comparison__elem">Вес</p>
					</div>

						<?php while( have_rows('robot_washer') ): the_row(); 
							$img = get_sub_field('img');
							$name = get_sub_field('name');

							$surface = get_sub_field('surface');
							$automatic_liquid_spraying = get_sub_field('automatic_liquid_spraying');
							$increase_the_cleaning_speed = get_sub_field('increase_the_cleaning_speed');
							$automatic_boundary_definition = get_sub_field('automatic_boundary_definition');
							$automatic_trajectory_detection = get_sub_field('automatic_trajectory_detection');
							$backup_power = get_sub_field('backup_power');
							$volume_of_the_reservoir_for_liquid = get_sub_field('volume_of_the_reservoir_for_liquid');
							$suction_power = get_sub_field('suction_power');
							$power_consumption = get_sub_field('power_consumption');
							$working_hours = get_sub_field('working_hours');
							$noise_level = get_sub_field('noise_level');
							$battery_capacity = get_sub_field('battery_capacity');
							$number_of_replacement_wipes = get_sub_field('number_of_replacement_wipes');
							$the_weight = get_sub_field('the_weight');
						...
						?>
							<div class="comparison__item">
								...
								<p class="comparison__elem"><?php echo $surface; ?></p>
								<p class="comparison__elem">
									<?php if($automatic_liquid_spraying){ ?>
										<img src="<?php echo get_template_directory_uri(); ?>/assets/img/checked.png" alt="">
									<?php } ?>
								</p>
								<p class="comparison__elem">
									<?php if($increase_the_cleaning_speed){ ?>
										<img src="<?php echo get_template_directory_uri(); ?>/assets/img/checked.png" alt="">
									<?php } ?>
								</p>
								<p class="comparison__elem">
									<?php if($automatic_boundary_definition){ ?>
										<img src="<?php echo get_template_directory_uri(); ?>/assets/img/checked.png" alt="">
									<?php } ?>
								</p>
								<p class="comparison__elem">
									<?php if($automatic_trajectory_detection){ ?>
										<img src="<?php echo get_template_directory_uri(); ?>/assets/img/checked.png" alt="">
									<?php } ?>
								</p>
								<p class="comparison__elem">
								...
								<p class="comparison__elem"><?php echo $volume_of_the_reservoir_for_liquid; ?></p>
								<p class="comparison__elem"><?php echo $suction_power; ?></p>
								<p class="comparison__elem"><?php echo $power_consumption; ?></p>
								<p class="comparison__elem"><?php echo $working_hours; ?></p>
								<p class="comparison__elem"><?php echo $noise_level; ?></p>
								<p class="comparison__elem"><?php echo $battery_capacity; ?></p>
								<p class="comparison__elem"><?php echo $number_of_replacement_wipes; ?></p>
								<p class="comparison__elem"><?php echo $the_weight; ?></p>
							</div>
						<?php endwhile; ?>
